...
class BinaryRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15,
                 cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0,cluster_people=None):
        ...
        self.cluster_people=cluster_people
        if cluster_people is not None:
            self.__user_dict_init__cluster()
        # embeds = {
        #     'ui_emb': ui_emb,
        ...
        # with open(os.path.join(DATA_DIR[data_name], 'UI_Interaction_data/user_item_dict_time.json'),
        #           encoding='utf-8') as f:
        #
        #     self.support_dict=json.load(f)
        # print('train_data: load RL support item dict')
        # print('people:',len(self.support_dict.keys()))

        embeds = load_embed(data_name, embed, epoch=fm_epoch)
        if embeds:
            self.ui_embeds =embeds['ui_emb'].cpu().detach()
            self.feature_emb = embeds['feature_emb'].cpu().detach()
        else:
    ...
    def __user_dict_init__cluster(self):
        ui_nums = 0
        self.user_weight_dict_cluster=dict()

        for user in self.cluster_people:
            self.user_weight_dict_cluster[str(user)]=self.ui_dict[str(user)]

        self.ui_dict = self.user_weight_dict_cluster
        print('user_dict cluster successfully!')
    ...
    def reset(self, embed=None,item=None,user=None):
        ...
        if self.mode == 'train':
            ...
            # self.user_id = np.random.choice(users, p=list(self.user_weight_dict.values())) # select user  according to user weights
                self.user_id = int(np.random.choice(users))
            else:
                self.user_id=user
            if item is not None:
        ...
        # support_item_list = self.support_dict[str(self.user_id)]
        # index = support_item_list.index(self.target_item)
        # support_item_list = support_item_list[:index]
        support_item_list=[]

        return self._get_state(), self._get_cand(), self._get_action_space(),self._map_to_all_id(self.cur_node_set,'feature')[0],support_item_list[-min(1,len(support_item_list)):]
